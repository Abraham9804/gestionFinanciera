<div x-data="{
    ...
    apiKeys: [{
            ...
            value: 'sk_live_**********4248',
            actualValue: 'sk_live_sjkhhjksdfkhkh4248',
            active: false,
            created: '25 Jan, 2025',
        ...
        },
        {
            ...
            actualValue: 'dev_live_abcdefghij4923',
            active: true,
            created: '29 Dec, 2024',
        ...
        },
        {
            ...
            actualValue: 'leg_live_klmnopqrst0932',
            active: true,
            created: '12 Mar, 2024',
    ...
    copyKey(index) {
        navigator.clipboard.writeText(this.apiKeys[index].actualValue).then(() => {
            this.apiKeys[index].copied = true;
            setTimeout(() => {
                this.apiKeys[index].copied = false;
            }, 2000);
        });
    },
    regenerateKey(index) {
        const chars = 'abcdefghijklmnopqrstuvwxyz0123456789';
        const prefix = this.apiKeys[index].value.split('_')[0];
        let actualKey = prefix + '_live_';
        let randomPart = '';
        for (let i = 0; i < 10; i++) {
        ...
        actualKey += lastFour;
        const maskedKey = prefix + '_live_' + '**********' + lastFour;
        this.apiKeys[index].value = maskedKey;
        this.apiKeys[index].actualValue = actualKey;
        this.apiKeys[index].lastUsed = 'Just now';
    },
    deleteKey(index) {
    ...
    editKey(index) {
        const newName = prompt('Enter new name for API key:', this.apiKeys[index].name);
        if (newName && newName.trim()) {
            this.apiKeys[index].name = newName.trim();
        }
    },
    generateApiKey() {
        ...
        <div
            ...
            <div>
                <button @click="isModalOpen = true"
                    class="bg-blue-500 hover:bg-blue-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium text-white transition">
                ...
                <div x-show="isModalOpen" x-cloak
                    ...
                    <div class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 lg:p-10 dark:bg-gray-900">
                        ...
                        <div>
                            <h4 class="text-2xl mb-1 font-semibold text-gray-800 dark:text-white/90">
                                Generate API key
                            </h4>
                            ...
                            <form @submit.prevent="generateApiKey">
                                <div>
                                    ...
                                    <input type="text" x-model="appName"
                                        class="shadow-theme-xs focus:border-blue-300 focus:ring-blue-500/10 dark:focus:border-blue-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                                </div>
                            ...
                            <div class="mt-8 flex w-full items-center justify-between gap-3">
                                ...
                                <button @click="generateApiKey" type="button"
                                    class="bg-blue-500 shadow-theme-xs hover:bg-blue-600 flex w-full justify-center rounded-lg px-4 py-3 text-sm font-medium text-white">
                                    Generate API key
                                </button>
                            </div>
                        </div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-800">
                        <th class="py-3 pr-5 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Name</th>
                        <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Status</th>
                        <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Created
                        </th>
                        <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Last used
                        </th>
                        ...
                        <tr>
                            <td class="py-3 pr-5 whitespace-nowrap">
                                <div>
                                    <label :for="'api-' + index"
                                        ...
                                        x-text="key.name"></label>
                                    <div class="flex items-center gap-3">
                                        <div class="relative">
                                            <input :value="key.value" type="text" :id="'api-' + index" readonly
                                                class="shadow-theme-xs h-11 w-full min-w-[360px] rounded-lg border border-gray-300 bg-transparent py-3 pr-[90px] pl-4 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
                                            <button type="button" @click="copyKey(index)"
                                                ...
                                                <div class="copy-text" x-text="key.copied ? 'Copied' : 'Copy'"></div>
                                            </button>
                                        </div>

                                        <div class="group relative inline-block">
                                            <button @click="regenerateKey(index)"
                            ...
                            <td class="px-5 py-3 whitespace-nowrap">
                                <span
                                    ...
                                    :class="key.active ? 'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500' :
                                        'bg-red-50 text-red-600 dark:bg-red-500/15 dark:text-red-500'"
                                    x-text="key.active ? 'Active' : 'Disabled'"></span>
                            </td>
                            <td class="px-5 py-3 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400"
                                x-text="key.created"></td>
                            <td class="px-5 py-3 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400"
                                x-text="key.lastUsed"></td>
                            <td class="px-5 py-3 whitespace-nowrap">
                                <div>
                                    <label :for="'toggle-' + index" class="cursor-pointer">
                                        <div class="relative">
                                            ...
                                            <div class="block h-6 w-11 rounded-full"
                                                :class="key.active ? 'bg-blue-500 dark:bg-blue-500' :
                                                    'bg-gray-200 dark:bg-white/10'">
                                            </div>
                                            <div :class="key.active ? 'translate-x-full' : 'translate-x-0'"
                            ...
                            <td class="px-5 py-3 whitespace-nowrap">
                                <div class="flex w-full items-center gap-3">
                                    <button @click="deleteKey(index)"
                                        class="hover:text-red-500 dark:hover:text-red-500 text-gray-500 dark:text-gray-400">
